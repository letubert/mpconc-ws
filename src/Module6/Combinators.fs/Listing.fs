...
let toByteArrayAsyncResult(image : Image) =
    async {
        use memStream = new MemoryStream()
...
// TODO : 2.18
// Leveraging the AsyncResult higher order functions for fluent composition
...
//    |> AsyncResult.map(fun image -> toByteArrayAsyncResult(image))
//    |> AsyncResult.bimap (fun bytes -> FileEx.WriteAllBytesAsync(destinationImage, bytes))
//                         (fun ex -> logger.Error(ex) |> async.Return)

// Using the AsyncResultBuilder
...
module ApplicativeFunctors =
    ...
    // Running in parallel a chain of operations using F# async Applicative Functor
...
module ``Composing and executing heterogeneous parallel computations`` =
    ...
    let analyze stockId =
        ...
        |> Async.StartContinuation(function
