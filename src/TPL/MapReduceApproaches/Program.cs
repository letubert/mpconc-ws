...
namespace MapReduceApproaches
{
    class Program
    {
        private const uint TopCount = 100;

        private static readonly FileInfo InputFile = new FileInfo(@"C:\ws\Data\plot.list");
        ...
        private static void Main()
        {
            ...
            var result = (uint)words.Sum(pair => pair.Value);
        ...
        private static IDictionary<string, uint> GetTopWordsPLINQProducerConsumer()
        {
            ...
            {
                foreach (var word in line.Split(Separators, StringSplitOptions.RemoveEmptyEntries).Where(w => !IsValidWord(w)))
